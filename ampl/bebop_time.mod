...
#Objective----------------------------------------------------
    ...
    param reg default 0; # whether to regularise or not

    param alpha1 default 0;
    param alpha2 default 0.1;

    param alpha1_1 := if reg=1 then alpha1 else 0;
    param alpha2_1 := if reg=1 then alpha2 else 0;

    var cost{i in I} = (alpha1_1*u1[i]*u1[i] + alpha2_1*u2[i]*u2[i]);
    var cost_m{i in J} = (alpha1_1*u1m[i]*u1m[i] + alpha2_1*u2m[i]*u2m[i]);
    var power = dt/6 * sum{i in J} (cost[i]+4*cost_m[i]+cost[i+1]);

    var time = tf + power;

    minimize myobjective: time;
